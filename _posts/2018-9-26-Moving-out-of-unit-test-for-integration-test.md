...
- I had to force the logger DI setup, I think it's set by something in ConfigureService
...
```
- This test will uses both the command and query
...
I call internal dependencies, dependencies that I manage entirely like CosmosDB. CosmosDB doesn't support transaction with multiple client request like SQL Server so I have to clean up the database after each tests. Here is my base class for doing this :
...
There is of course dawbacks in this way of testing :
...
- You have to be able to clean all the dependencies between each test, if not they fail in the "I don't manage" this dependency
