...
On my [Toss project](https://github.com/RemiBou/Toss.Blazor), I chose to have some e2e test, those kind of tests have many drawbacks :
- Force you to add ids everywhere on your html code
- Are often flaky
...
At first I started my test with Selenium. Mainly because it's the most common way to do this and because there was some example on the aspnet core repo. But Selenium test are hard to write and very flaky (just look at my [git logs](https://github.com/RemiBou/Toss.Blazor/commits/master)) for many reason :
...
- You can use the browser just like any user so you can't add code for waiting the end of an http query or check the status of a xhr.
- There isn't much debugging informatin provided : screenshot are hard to get and videos impossible.

I heard a lot of good thing about [cypress](https://www.cypress.io/) so I decided to give it a shot after an not successful battle against Selenium.

## Start the application and dependencies

Because my E2E test were done with Selenium it was a .net project, so I was bale to launch RavenEB.Embedded. Cypress running on node, I will get rid of this project, so I decided to use docker-compose. 
I really like docker and it's a common way to do this. For running my app in a docker compose I need to build a docker image for it, here it is
...
```
 - Docker is really nice because I was able to easily import the 2.2.7 runtime into a 3.0 sdk because my integration test runs RavenDB.Embedded which is not compatible with runtime 3.
 - With multi stage build I can use the sdk for building my app then only the runtime for executing it.
 - I first copy the csproj and sln files for uusing layer caching : if I don't change anything to my csproj files, then the "RUN dotnet restore ./Toss.sln" line will use the cached version making my build faster.
...
- The good thing with docker compose is that they run on they own network with service accessible via their name, so I can run ravendb inside this network and it won't have an impact on the other ravendb I might be running on my dev computer.
...
Maybe I'll use it as a starting point for hosting my app in production but it was not the point.

## Cypress Test

First I create the folder "Toss.TEsts.E2E.Cypress" in my solution then cd on it. You need npm installed on your computer. Type on your terminal :
...
Once installed I got a cypress folder containing multiple folder, I cleaned up the folder called "integration". And created my own test using javascript, and cypress :
...
- Cypress uses mocha for running the tests, so your test fixture must be a call to "describe()" function and inside each test is a call to a "it" function. As in every test runner, there is hook for running things before each tests.
- Cypress is able to read every XHR request done by your site but Blazor uses fetch for http call, so we need to remove the current window implementation of fetch and replace it by a polyfill that uses xhr.
- We can see with the route() and wait() call how you can wait until an http request is done or even how we can read its content : here I needed a way to get the confirmation link after a subscribtion, so server side I send this link in the http response header (only in test mode) and I read it client side and then redirect the user.
- For E2E I prefer to write one big test that does a lot of thing, it's just a mater of taste.
...
Here is the catch : on my e2e test if I wrote "win.runCaptcha = function(action){return Promise.resolve(action);}" it would throw an error "The value 'window.runCaptcha' is not a function." because class definition (like Function) are scoped by window so my mthod would be a function in the scope of the cypress window, not on the scope of the tested app window (it took me no less than 2 days to figure it out).
...
Which opens a web UI where I can run my test and see them while they are executing. This GUI is really nice because you can go in the past and see the state of the UI or even browse the DOM. Now that my test passes I want to integrate cypress into my docker-compose so I can run it without event installing cypress, which will help when I'll run it in my CI environment.
...
- I mount the cypress tests as a volume, so i can also get back the cypress test execution artifacts (screenshot and artifacts)
- The depends on is really usefull for starting things in order
...
Then for running the test I run the following command from the rot fo my project

```bash
docker-compose up --renew-anon-volumes --exit-code-from cypress --build
```

- "--renew-anon-volumes" is used for cleaning ravendb volumes before each run, so I start with a clean DB.
...
Now that I got a fully running integration I can replace the existing one for this. Because it uses docker-compose it'll be easier to maintain (I can test it on my dev machine) and will also help for running other servcies. Here is my new "azure-pipelines.yml" file
...
- I use a ubuntu image because it's lighter than windows and I don't think I'll run my app on windows server so it's better to do my e2e test on a linux OS.
- You need to create a docker endpoint on Azure DevOps so it can login docker hub for pulling your images
...
I can also say that Cypress is way better than Selenium in every possible way : 
- installation : a single docker service or an npm package
- development : no need to add wait everywhere
